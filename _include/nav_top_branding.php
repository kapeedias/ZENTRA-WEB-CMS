<nav class="navbar bg-body rounded-4 shadow-sm px-2">
    <div class="container-fluid"><a class="text-decoration-none link-body-emphasis d-inline-flex align-items-center"
            href="/myaccount.php"><span class="fs-4 fw-bold brand-primary">ZENTRA</span><span
                class="fs-4 brand-secondary">CMS</span></a><button class="navbar-toggler border-0"
            data-bs-toggle="offcanvas" data-bs-target="#sidebar"><span class="visually-hidden">Toggle
                navigation</span><span class="navbar-toggler-icon"></span></button></div>
</nav>
